...
调价待办拒绝入口是 `scripts/temu-price-adjust-reject.mjs`。它默认从 `https://agentseller.temu.com/goods/list` 建立 agentseller 登录/授权态；业务数据和拒绝动作全部走 API，不打开调价管理页、不勾选表格、不打开拒绝弹窗、不点击页面按钮，也不通过页面店铺切换器逐店切换。脚本先通过 `POST /api/seller/auth/userInfo` 精确匹配店铺名取得 `mallId`，再带 `mallid` 请求头调用 `POST /api/kiana/magnus/mms/price-adjust/page-query`，固定请求体 `{"pageInfo":{"pageSize":200,"pageNo":页码},"status":1}` 并翻页收集内部调价 `id`；提交接口为 `POST /api/kiana/magnus/mms/price-adjust/batch-review`，payload 形状是 `{"batchResult":2,"submitOrders":[调价单id],"rejectReasons":{"调价单id":"0"}}`。拒绝原因固定为 `0`。不要实现或调用 `batchResult:1`、`批量确认`、`调整` 或任何同意调价路径。

正式交付路径只使用企业微信群机器人 webhook：`WECOM_WEBHOOK_URL`。`npm run temu:report:all:image` 先生成合并 JSON，再生成 780px 宽的紧凑汇总图片；today 流程把销售汇总、出库异常摘要和今日新增异常 SPU 摘要作为第一条 markdown 发送，随后发送汇总图片，不再单独发送出库异常、店铺运营状态和今日新增异常 SPU `markdown_v2` 表格；yesterday 流程发送昨日销售汇总 markdown、图片和店铺资金明细图片表格，不再发送店铺运营状态表格；不要回退到 `wecom-cli` 文本发送。
...
- `scripts/temu-operation-status-markdown.mjs`: 出库异常和店铺运营状态 `markdown_v2` 表格发送
- `scripts/temu-shop-funds-image.mjs`: 店铺资金明细图片表格生成和发送
...
5. 看最新 `temu-operation-status-*.json`，确认每店 `regionCounts`、`inSaleSpuCount` 和 `missingInSaleProducts`。
6. 如手动执行过调价拒绝，再看最新 `temu-price-adjust-reject-*.json`，确认只执行了拒绝且剩余为 0。
...
- 企业微信只通过 `WECOM_WEBHOOK_URL` 群机器人 webhook 发送，不使用 `wecom-cli`；today 流程发送顺序为销售汇总 markdown、销售汇总图片，不再单独发送出库异常和店铺运营状态 `markdown_v2` 表格；yesterday 流程发送顺序为昨日销售汇总 markdown、销售汇总图片、店铺资金明细图片表格
...
- `TEMU_REPORT_DATE=yesterday` 时，广告数据完成后会执行 setonr组、whitine组 和 WONDER 店铺资金采集，并单独发送企业微信资金明细图片表格；表头为 `店铺名`、`可提现金额`、`待处理/待结算`、`资金合计`，最后一行为 `合计`
- `TEMU_REPORT_DATE=today` 时，广告数据完成后会通过 `scripts/temu-run-agentseller-checks.mjs` 按顺序执行出库单异常巡店和店铺运营状态巡店；调价待办拒绝暂停每日自动执行，只保留手动命令；出库单异常全部为 0 时第一条 markdown 显示 `今日出库单异常0条。`，否则逐店显示异常条数
- agentseller runner 只做顺序调度，不并发、不共享同一个标签页；两个子脚本仍使用同一个账号 Chrome profile，因此登录态一致，但各自按原有逻辑完整打开和关闭 CDP Chrome
- 出库异常和运营状态表格使用企业微信 webhook `markdown_v2` 类型，因为普通 `markdown` 不支持表格；资金明细使用图片表格，避免手机端 `markdown_v2` 表格换行
- 运营状态主表表头固定为 `店铺`、`在售SPU数`
- 新增异常 SPU 指上一份快照中在售、当前快照中不在售或缺失的 SPU ID；如果存在异常，第一条 markdown 追加 `今日新增异常SPU` 摘要，写明店铺名和 SPU ID；上一份在售 SPU 列表为空时自然不报异常，也不会追加该段
...
按顺序执行出库单异常巡店和店铺运营状态巡店，只生成 JSON，不发送企业微信；调价待办拒绝暂停每日自动执行：
...
从最新店铺运营状态 JSON 发送或预览企业微信 `markdown_v2` 表格；可通过 `--abnormal-input` 附加出库异常表格：
...
- `npm run temu:report:all:image` 生成紧凑汇总图片，并通过企业微信群机器人 webhook 发送销售汇总 markdown 和图片；today 流程的出库异常摘要和今日新增异常 SPU 摘要写在第一条 markdown；yesterday 流程额外发送店铺资金明细表格
...
- `00:01`：通过 API 时间戳汇总并推送昨日数据，标题使用 `昨日汇总`，并追加店铺资金明细表格
...
`09:00` 的 `today` 流程中，广告数据跑完后会用 `npm run temu:agentseller-checks` 按顺序跑出库单异常和店铺运营状态；调价待办拒绝暂停每日自动执行。两个检查完成后发送销售汇总 markdown 和图片；出库异常摘要以及今日新增异常 SPU 的店铺名和 SPU ID 写在第一条 markdown，不再单独发送出库异常、店铺运营状态和今日新增异常 SPU 表格。
